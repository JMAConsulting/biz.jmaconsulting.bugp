...
class CRM_Grant_BAO_Query {
  ...
  static function whereClauseSingle(&$values, &$query) {
    ...
    switch ($name) {
      ...
      case 'grant_type_id':
        // The grant_type_id might be an array (from aggregate grants custom search)
        ...
        $types = CRM_Core_OptionGroup::values('grant_type');
        ...
        $query->_where[$grouping][] = CRM_Contact_BAO_Query::buildClause("civicrm_grant.grant_type_id",
          $op, $value, "Integer"
        );
        $query->_qill[$grouping][] = ts('Grant Type %1', array(1 => $op)) . ' ' . implode(' ' . ts('or') . ' ', $names);
        $query->_tables['civicrm_grant'] = $query->_whereTables['civicrm_grant'] = 1;
        return;

      case 'grant_status_id': 
        
        // The grant_status_id might be an array (from aggregate grants custom search)
        // In this case, we need to change the query.
        if (is_array($value)) {
          $val = array();
          // Rebuild the array to get the data we're interested in as array
        ...
        $types = CRM_Core_OptionGroup::values('grant_status');
        ...
        $query->_where[$grouping][] = CRM_Contact_BAO_Query::buildClause("civicrm_grant.status_id",
          $op, $value, "Integer"
        );
        $query->_qill[$grouping][] = ts('Grant Status %1', array(1 => $op)) . ' ' . implode(' ' . ts('or') . ' ', $names);
        $query->_tables['civicrm_grant'] = $query->_whereTables['civicrm_grant'] = 1;
