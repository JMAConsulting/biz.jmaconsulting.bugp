...
class CRM_UF_Form_Field extends CRM_Core_Form {
  ...
  public function preProcess() {
    ...
    if (CRM_Core_Permission::access('CiviContribute')) {
      ...
      $this->_fields = array_merge(CRM_BUGP_BAO_UFField::getContribBatchEntryFields(), $this->_fields);
    }
    ...
    $this->_hasSearchableORInSelector = CRM_BUGP_BAO_UFField::checkSearchableORInSelector($this->_gid);
  ...
  public function buildQuickForm() {
    ...
    if (isset($this->_id)) {
      $params = ['id' => $this->_id];
      CRM_BUGP_BAO_UFField::retrieve($params, $defaults);
      ...
      $specialFields = array_merge(CRM_BUGP_BAO_UFGroup::getLocationFields(), $addressCustomFields);
    ...
    $otherModules = array_values(CRM_BUGP_BAO_UFGroup::getUFJoinRecord($this->_gid));
    ...
    $fields = CRM_BUGP_BAO_UFField::getAvailableFields($this->_gid, $defaults);
    ...
    if (CRM_BUGP_BAO_UFField::checkProfileType($this->_gid)) {
  ...
  public function postProcess() {
    ...
    if ($this->_action & CRM_Core_Action::DELETE) {
      ...
      $deleted = CRM_BUGP_BAO_UFField::del($this->_id);

      //update group_type every time. CRM-3608
      if ($this->_gid && $deleted) {
        //get the profile type.
        $fieldsType = CRM_BUGP_BAO_UFGroup::calculateGroupType($this->_gid, TRUE);
        CRM_BUGP_BAO_UFGroup::updateGroupTypes($this->_gid, $fieldsType);
      }
    ...
    if ($apiFormattedParams['field_type'] != "Formatting" && CRM_BUGP_BAO_UFField::duplicateField($apiFormattedParams)) {
      CRM_Core_Error::statusBounce(ts('The selected field already exists in this profile.'), NULL, ts('Field Not Added'));
    }
    else {
      $apiFormattedParams['weight'] = CRM_BUGP_BAO_UFField::autoWeight($params);
      ...
      if ($this->_hasSearchableORInSelector &&
        ...
        CRM_BUGP_BAO_UFField::resetInSelectorANDSearchable($this->_gid);
      }
  ...
  public static function formRuleCustomDataExtentColumnValue($customField, $gid, $fieldType, &$errors) {
    ...
    if (in_array($fieldType, [
      ...
      $fieldTypeValues = CRM_BUGP_BAO_UFGroup::groupTypeValues($gid, $fieldType);
  ...
  public static function formRule($fields, $files, $self) {
    ...
    if ($profileFieldName) {
      ...
      if ($customFieldId[0] == 'custom') {
        ...
        if ($customGroupId) {
          if ($profileMultiRecordCustomGid = CRM_BUGP_BAO_UFField::checkMultiRecordFieldExists($self->_gid)) {
    ...
    $groupFields = CRM_BUGP_BAO_UFGroup::getFields($fields['group_id'], FALSE, NULL, NULL, NULL, TRUE, NULL, TRUE);
    ...
    if (CRM_BUGP_BAO_UFGroup::isProfileDoubleOptin()) {
      if (CRM_Utils_Array::value(1, $fields['field_name']) == 'group') {
        $dao = new CRM_BUGP_BAO_UFField();
    ...
    //get the group type.
    $groupType = CRM_BUGP_BAO_UFGroup::calculateGroupType($self->_gid, FALSE, CRM_Utils_Array::value('field_id', $fields));
    ...
    switch ($fieldType) {
      ...
      default:
        $profileType = CRM_BUGP_BAO_UFField::getProfileType($fields['group_id'], TRUE, FALSE, TRUE);
